...
public class Placement {
	private static class Department implements Comparable<Department> {
		...
		public int compareTo(Department other) {
			return Comparator.comparingInt(Department::getPlacements).compare(this, other);
		}
	...
	public static void main(String[] args) {
		...
		if (cs < 0 || ec < 0 || me < 0) {
			...
			if (departmentSet.size() == 1) {
				...
				departmentArrayList.stream().filter(
						department -> department.placements == Collections.max(departmentArrayList).getPlacements())
